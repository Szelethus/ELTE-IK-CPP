...
int main()
{
	...
	\end{lstlisting}
	\section{Referencia szerinti paraméter átvétel}
...
void swap(int * const a, int * const b)
{
	...
	\end{lstlisting}
	Egy kis szintaktikai cukorkával megúszhatjuk azt, hogy folyton kiírjuk a \texttt{* const}-ot (lévén ritkán akarjuk megváltoztatni hogy a tömb hova mutasson). Erre való a {referencia szerinti paraméter átváétel} (\textit{pass by reference}). A referencia úgy működik, mintha egy konstans pointer lenne.
	\begin{lstlisting}
void swapRef(int &a, int &b)
{
	...
	\begin{note}
		Ez bár ez referencia szerinti átvétel, de itt is történik másolás, a memóriacímet így is érték szerint vesszük át.
	\end{note}
...
int main()
{
	...
	Amikor létrehozunk egy \texttt{Hallgato} típusú objektumot fentebb, akkor minden adattagjára meghívódik a default konstruktor (erről bővebben 6. gyakorlaton).
	
	\texttt{int}-ek alapértezetten memóriaszemetet kapnak.
	\begin{lstlisting}
int i;
...
int main() 
{
	...
	\end{lstlisting}   
	Itt ha kiíratjuk \texttt{i}-t, mégis mindig 0-t fogunk kapni. Ennek oka az, hogy \texttt{i} egy globális változó, és a globális változók mindig 0-ra inicializálódnak (legalábbis az \texttt{int}-ek). A globális változókat csak egyszer hozzuk létre a program futásakot, így érdemes jól definiált kezdőértéket adni neki.
	
	 Azonban a stacken rengetegszer létre kell hozni változókat, nem csak egyszer, így ott nem éri meg minden alkalommal egy jól definiált kezdőértékkel inicializálni. Sokkal nagyobb lenne a hatása a futási időre.
	
	Az hogy miért épp 0-ra inicializálódnak, azért van, mert mert az nagyon gyors, és determinisztikus. Így hibákat is egyszerűbb kiszűrni, hisz egy globálisról mindig tudjuk hogy 0-ra inicializál alapértelmezetten.
