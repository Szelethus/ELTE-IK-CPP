...
\begin{lstlisting}
class Negyzet : virtual public Deltoid, virtual public Rombusz
{
public:
	double terulet()
	{
...
void f(Sikidom *s)
{
	...
	A \texttt{dynamic\_cast} futási időben ellenőriz, és ha a cast nem sikeres, nullpointert ad vissza. Működését úgy képzelhetjük el, hogy megnézi a paraméterként adott objektum típusát, és végighaladván az öröklődési ágon, megpróbál egy helyes típuskonverziót csinálni. (A következő példában tekintsünk el attól, hogy a konstruktoroknak nem adtunk paramétert)
	
	A \texttt{dynamic\_cast} csak polimorfikus osztályoknál használható. 
	%TODO referenciák castolása
	%TODO sidecast
	\begin{lstlisting}
int main()
{
	...
	sp = &k; //ok
	sp = dynamic_cast<Sikidom*>(&k); //folosleges, de ok
	
	kp = &s; //nem ok
	kp = dynamic_cast<Kor*>(&s); //ok, kp most nullpointer
	
	kp = dynamic_cast<Kor*>(sp); //ok, kp k-ra mutat
}
	\end{lstlisting}
	\begin{note}
	...
	Ez azonban nem hatékony, hisz futási időben kell végigjárnia az öröklődési láncot. Ha nem vagyunk biztosak benne, hogy garantáltan lehetséges a castolás, akkor \texttt{dynamic\_cast}-ot érdemes használni, ha azonban biztosak vagyunk benne hogy az lehetséges, akkor \texttt{static\_cast} a jobb, mely fordítási időben végzi el ezt, hatékonyabb, azonban kevésbé biztonságos, ugyanis ha a cast sikertelen, akkor nem nullpointert ad vissza, hanem egy nem definiált viselkedés fog bekövetkezni (leggyakrabban elszáll).
...
public:
	...
	\smallskip
	Hasonlóan hasznol kulcsszó a \texttt{final}, mely megtiltja az öröklődést, így bátran használhatunk nem virtuális destruktorokat.
...
class Derived : public Base {}; //forditasi ideju hiba
	\end{lstlisting}
	Amennyiben örököltünk egy osztályból, mely privát adattagokat tartalmaz, azokhoz nem fogunk tudni hozzáférni. Ilyenkor rá vagyunk kényszerülve get/set függvények alkalmazására, így ezért is érdemes lehet őket megírni.
...
public:
	...
	A slicing-ről már volt szó, azonban veszélyesebb az talán, mint elsőre gondolnánk. EMlékeztetőként, akkor fordul ez elő, ha egy objektumot egy, a bázisosztály objektuménak adjuk \textbf{értékül}. Ilyenkor minden speciális adat elveszik, nem fogunk tudni hivatkozni specifikus metódusokra, de ami még veszélyesebb, hogy a virtuális függvényeket uyganőgy meg fogjuk tudni hívni, azonban azok mást fognak cisnálni, mint amire számítunk.
	\begin{lstlisting}
struct Base
{
