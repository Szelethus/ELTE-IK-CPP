...
%	\begin{note}
		%TODO számliterálokhoz kéne átrakni
...
public:
	...
	\subsection{Hozzáférés típusa (\textit{access specifier})}
	...
	\end{note}
	Az, hogy publikusan örököltünk, azt is jelenti, hogy egy {altípus}-t (\textit{subtype}) hoztunk létre, és mint egy altípus hogy minden helyre, ahol \texttt{Sikidom}-ot szeretnénk használni, \texttt{Kor}-t is használhatunk. 
	\begin{lstlisting}
Kor k;
Sikidom s;
s = k; //ok
...
s.y = k.y;
	\end{lstlisting}
	Ez azonban magával hordozza azt is, hogy minden információ, ami \texttt{Kor}-re specifikus (pl. sugár (\texttt{r})) elveszik. Ez a folyamatot \textit{slicing}. Ebben az esetben, lévén a fordító generálta nekünk az értékadó operátort, és az alapér-nak nevezzük.
...
\end{lstlisting}
	...
	\section{Cast-ok}
	%TODO : privát öröklődés és reinterpret cast
